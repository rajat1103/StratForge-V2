...
export async function GET(req: NextRequest) {
  ...
  const [exams, streak, insights] = await Promise.all([
    prisma.exam.findMany({
      ...
      include: {
        topics: {
          select: {
            masteryLevel: true, status: true, priority: true,
            title: true, difficulty: true, id: true,
          },
        },
      },
    }),
  ...
  const weeklyProgress = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      day: days[d.getDay()],
      minutes: dayLogs.reduce((s, l) => s + l.minutesSpent, 0),
      target: 120,
    }
  })
  ...
  const todayMinutes = weekLogs
    .filter(l => new Date(l.logDate) >= today)
  ...
  const avgCompletion =
    exams.length > 0
      ? Math.round(
          exams.reduce((s, e) => {
            const done = e.topics.filter(t => t.status === 'completed').length
  ...
  const upcomingDeadlines = exams
    ...
    .map(e => ({
      ...
      daysLeft: Math.max(
        0,
        Math.ceil((new Date(e.examDate!).getTime() - Date.now()) / 86400000)
      ),
      completionPct: e.topics.length
        ? Math.round(
            (e.topics.filter(t => t.status === 'completed').length /
              e.topics.length) *
              100
          )
        : 0,
  ...
  const topicHeatmap = exams.flatMap(e =>
    e.topics.map(t => ({
      topicId: t.id,
      topicTitle: t.title,
      examTitle: e.title,
